...
import random
from datasets.label_json import reset_subset
import logging
...
## 将['start','preparation','end']转成[-1,0,1]
...
def make_finetune_dataset(root_dir, annotation_path, subset):
    ...
    for item in database:
        ...
        for img_index in img_slice:
            sample = {
                'path': os.path.join(root_dir, item['video'], 'image_{:08d}.jpg'.format(img_index)),
                'regression': img_index/item['n_frames']
                    }
            dataset.append(sample)
...
def make_guide_dataset(root_dir, annotation_path, subset, n_sample_for_each_video, sample_duration):
    ...
    for i in pbar:
        ...
        if sample['next'] != 'End' and sample['next'] != 7 \
            ...
            pre_sample = get_related_sample(sample, full_data, is_previous=True) #获得上一片段视频信息
            future_sample = get_related_sample(sample, full_data, is_previous=False) #获得下一片段视频信息
    ...
    data_dist = reset_subset(data_length) #随机分训练集，验证集
...
class CholecData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_finetune:  # finetune
            ...
            target = self.data[index]['regression']
