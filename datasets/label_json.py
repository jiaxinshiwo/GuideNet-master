...
def make_json_file(label_path, txt_label_path, file_name):
    ...
    for item in label_data.keys():
        ...
        frames=np.arange(1, len(original_flow) + 1).tolist()
        train_slice, test_slice, val_slice = get_finetune(frames)
        train_regression=[x/len(original_flow) for x in train_slice]
        test_regression = [x/len(original_flow) for x in test_slice]
        val_regression = [x/len(original_flow) for x in val_slice]
        sample = {
            'video': item,
            'n_frames': len(frames),
...
if __name__ == '__main__':
    label = '/home/amax/lab_data/surgicalData/cholec80_data/phase_annotations/data.pkl'
    txt_label = '/home/amax/lab_data/surgicalData/cholec80_data/phase.txt'
    set_name = 'data'
